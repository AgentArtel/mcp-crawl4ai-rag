-- Fixed Authentication and User Profile Schema for Supabase
-- This schema works with Supabase's built-in authentication system
...
-- Add foreign key constraint separately (safer for Supabase)
DO $$ 
BEGIN
    IF NOT EXISTS (
...
DROP POLICY IF EXISTS "Users can view own course history" ON student_course_history;
DROP POLICY IF EXISTS "Users can manage own course history" ON student_course_history;
DROP POLICY IF EXISTS "Users can manage own graph preferences" ON student_graph_preferences;
...
DROP TRIGGER IF EXISTS update_student_course_history_updated_at ON student_course_history;
DROP TRIGGER IF EXISTS update_student_graph_preferences_updated_at ON student_graph_preferences;
